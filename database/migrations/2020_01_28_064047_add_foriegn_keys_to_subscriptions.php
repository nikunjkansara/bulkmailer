...
class AddForiegnKeysToSubscriptions extends Migration
{
    ...
    public function up()
    {
        ...
        \horsefly\Cashier\Subscription::whereRaw(sprintf('(%s NOT IN (SELECT uid FROM %s) OR %s NOT IN (SELECT uid FROM %s))', 'user_id', table('customers'), 'plan_id', table('plans')))->delete();
