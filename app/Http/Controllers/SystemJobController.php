...
class SystemJobController extends Controller
{
    ...
    {
        $system_jobs = \horsefly\Model\SystemJob::search($request)->paginate($request->per_page);
    ...
    {
        $job = \horsefly\Model\ImportSubscribersSystemJob::find($request->id);

        // authorize
        // @todo: redundant find() here
        if (\Gate::denies('downloadImportLog', \horsefly\Model\SystemJob::find($request->id))) {
    ...
    {
        $jobs = \horsefly\Model\SystemJob::whereIn('id', explode(',', $request->uids));
    ...
    {
        $job = \horsefly\Model\SystemJob::find($request->id);
