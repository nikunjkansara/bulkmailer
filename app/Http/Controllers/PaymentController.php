...
use horsefly\Model\Payment;
use horsefly\Library\Log as PaymentLog;
...
class PaymentController extends Controller
{
    ...
    public function billingInformation(Request $request, $subscription_uid)
    {
        ...
        $subscription = \horsefly\Model\Subscription::findByUid($subscription_uid);
        $billing_information = [];
    ...
    public function paypal(Request $request, $subscription_uid)
    {
        ...
        $payment_method = \horsefly\Model\PaymentMethod::getByType(\horsefly\Model\PaymentMethod::TYPE_PAYPAL);
        ...
        if ($request->isMethod('post')) {
            try {
                ...
                $payment->status = $result->success ? \horsefly\Model\Payment::STATUS_SUCCESS : \horsefly\Model\Payment::STATUS_FAILED;
    ...
    public function braintree_credit_card(Request $request, $subscription_uid)
    {
        ...
        $payment_method = \horsefly\Model\PaymentMethod::getByType(\horsefly\Model\PaymentMethod::TYPE_BRAINTREE_CREDIT_CARD);
    ...
    public function braintree_paypal(Request $request, $subscription_uid)
    {
        ...
        $payment_method = \horsefly\Model\PaymentMethod::getByType(\horsefly\Model\PaymentMethod::TYPE_BRAINTREE_PAYPAL);
    ...
    {
        $subscription = \horsefly\Model\Subscription::findByUid($subscription_uid);
        $payment_method = \horsefly\Model\PaymentMethod::getByType(\horsefly\Model\PaymentMethod::TYPE_CASH);
        ...
        $payment->subscription_id = $subscription->id;
        $payment->payment_method_id = $payment_method->id;
        $payment->status = \horsefly\Model\Payment::STATUS_CASH_MANUAL_CONFIRMATION;
        $payment->action = \horsefly\Model\Payment::ACTION_PAID;
        $payment->payment_method_name = trans('messages.' . $payment_method->type);
    ...
    public function stripe_credit_card(Request $request, $subscription_uid)
    {
        ...
        $payment_method = \horsefly\Model\PaymentMethod::getByType(\horsefly\Model\PaymentMethod::TYPE_STRIPE_CREDIT_CARD);
    ...
    public function paddle_card(Request $request, $subscription_uid)
    {
        ...
        $payment_method = \horsefly\Model\PaymentMethod::getByType(\horsefly\Model\PaymentMethod::TYPE_PADDLE_CARD);
        $vendorId = $payment_method->getOption('vendor_id');

        // Checkout url
        $data['vendor_id'] = $vendorId;
        $data['vendor_auth_code'] = $payment_method->getOption('vendor_auth_code');
        if ($subscription->plan->paddle_plan_id!="" || $subscription->plan->paddle_plan_id!=null) {
    ...
    public function paddle_card_hook(Request $request, $subscription_uid)
    {
        ...
        if ($verified) {
            ...
            $payment->status = \horsefly\Model\Payment::STATUS_SUCCESS; // : \horsefly\Model\Payment::STATUS_FAILED;
    ...
    public function payumoney(Request $request, $subscription_uid)
    {
        ...
        $payment_method = \horsefly\Model\PaymentMethod::getByType(\horsefly\Model\PaymentMethod::TYPE_PAYU_MONEY);
        $rules = [
            'name' => 'required',
            'emails' => 'required',
            'phones'=>'required',
    ...
    public function payumoney_fail(Request $request, $subscription_uid)
    {
        ...
        try {
            ...
            if ($request->isMethod('post')) {
                ...
                $payment->data = serialize($result);
                $payment->payment_method_id = $payment_method->id;
                $payment->status = \horsefly\Model\Payment::STATUS_FAILED;
                $payment->action = \horsefly\Model\Payment::ACTION_PAID;
                $payment->payment_method_name = trans('messages.' . $payment_method->type);
                $payment->order_id = $order_id;
