...
class Subscriber extends Model
{
    ...
    {
        return $this->hasMany('horsefly\Model\EmailVerification');
    }

    public function emailVerification()
    {
        return $this->hasOne('horsefly\Model\EmailVerification');
    }
    ...
    {
        return $this->hasMany('horsefly\Model\SubscriberField');
    }
    ...
    {
        return $this->hasMany('horsefly\Model\UnsubscribeLog');
    }
    ...
    public static function import($user, $list)
    {
        ...
        foreach ($lines as $key => $line) {
            ...
            // authorize
            if ($user->cannot('create', new \horsefly\Model\Subscriber(['mail_list_id' => $list->id]))) {
                ...
                if ($valid) {
                    ...
                    foreach ($parts as $key => $value) {
                        ...
                        if (isset($headers[$key])) {
                            ...
                            if (is_object($lf)) {
                                //// save fields
                                $lfv = new \horsefly\Model\SubscriberField(array(
                                                'field_id' => $lf->id,
    ...
    public function copy($list, $type = 'keep')
    {
        ...
        if (!is_object($copy)) {
            ...
            $copy = \horsefly\Model\Subscriber::find($copy->id);
        } else {
            // return if keep
            if ($type == 'keep') {
                return false;
    ...
    {
        $query = \horsefly\Model\TrackingLog::where('tracking_logs.subscriber_id', '=', $this->id);
    ...
    {
        $query = \horsefly\Model\OpenLog::leftJoin('tracking_logs', 'tracking_logs.message_id', '=', 'open_logs.message_id')
    ...
    {
        $query = \horsefly\Model\ClickLog::leftJoin('tracking_logs', 'tracking_logs.message_id', '=', 'click_logs.message_id')
            ->where('tracking_logs.subscriber_id', '=', $this->id);
