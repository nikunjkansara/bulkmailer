...
class Admin extends Model
{
    ...
    {
        return $this->belongsTo('horsefly\Model\AdminGroup');
    }

    public function customers()
    {
        return $this->hasMany('horsefly\Model\Customer');
    }

    public function templates()
    {
        return $this->hasMany('horsefly\Model\Template');
    }
    ...
    {
        return $this->belongsTo('horsefly\Model\User', 'creator_id');
    }
    ...
    public function getColorScheme()
    {
        if (!empty($this->color_scheme)) {
            ...
            return \horsefly\Model\Setting::get('backend_scheme');
        }
    }
    ...
    {
        $query = \horsefly\Model\Customer::getAll();

        if (!$this->user->can('readAll', new \horsefly\Model\Customer())) {
    ...
    {
        $query = \horsefly\Model\Log::select('logs.*')->join('customers', 'customers.id', '=', 'logs.customer_id')
    ...
    {
        return \horsefly\Model\Plan::getAllActive($this);
    }
    ...
    {
        $query = \horsefly\Model\PaymentMethod::getAll()
            ->where('payment_methods.status', '=', \horsefly\Model\PaymentMethod::STATUS_ACTIVE);

        if (!$this->can('readAll', new \horsefly\Model\PaymentMethod())) {
    ...
    {
        $query = \horsefly\Model\Admin::getAll()
            ->where('admins.status', '=', \horsefly\Model\Admin::STATUS_ACTIVE);

        if (!$this->can('readAll', new \horsefly\Model\Admin())) {
    ...
    {
        $query = \horsefly\Model\AdminGroup::getAll();

        if (!$this->can('readAll', new \horsefly\Model\AdminGroup())) {
    ...
    {
        $query = \horsefly\Model\SendingServer::getAll();

        if (!$this->can('readAll', new \horsefly\Model\SendingServer())) {
    ...
    {
        $query = \horsefly\Model\SendingDomain::getAll();

        if (!$this->can('readAll', new \horsefly\Model\SendingDomain())) {
    ...
    {
        $query = \horsefly\Model\Campaign::getAll();

        if (!$this->can('readAll', new \horsefly\Model\Customer())) {
    ...
    {
        $query = \horsefly\Model\Automation2::query();
    ...
    {
        $query = \horsefly\Model\Subscriber::getAll();
    ...
    public function addEmaillToBlacklist($email)
    {
        ...
        if (\horsefly\Library\Tool::isValidEmail($email)) {
            $exist = \horsefly\Model\Blacklist::whereNull('customer_id')->where('email', '=', $email)->count();
            if (!$exist) {
    ...
    public function getSubaccountSendingServers()
    {
        ...
        $query = $query->whereIn('type', \horsefly\Model\SendingServer::getSubAccountTypes());
