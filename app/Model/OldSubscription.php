...
class OldSubscription extends Model
{
    ...
    public $statuses = [
        self::STATUS_ACTIVE => [
            self::TIME_STATUS_PAST => 'expired',
            self::TIME_STATUS_CURRENT => 'current_active',
            self::TIME_STATUS_FUTURE => 'pending_active',
        ],
        self::STATUS_INACTIVE => [
            self::TIME_STATUS_PAST => 'expired',
            self::TIME_STATUS_CURRENT => 'current_inactive',
            self::TIME_STATUS_FUTURE => 'pending_inactive',
        ],
        self::STATUS_DISABLED => [
            self::TIME_STATUS_PAST => 'expired',
            self::TIME_STATUS_CURRENT => 'current_disabled',
            self::TIME_STATUS_FUTURE => 'pending_disabled',
        ],
    ];
    ...
    public function rules()
    {
        ...
        if ($this->getOption('sending_server_option') == \horsefly\Model\Plan::SENDING_SERVER_OPTION_SUBACCOUNT) {
    ...
    {
        return $this->belongsTo('horsefly\Model\Plan');
    }
    ...
    {
        return $this->belongsTo('horsefly\Model\PaymentMethod');
    }
    public function subscriptionsSendingServers()
    {
        return $this->hasMany('horsefly\Model\SubscriptionsSendingServer');
    }
    public function payments()
    {
        return $this->hasMany('horsefly\Model\Payment');
    }
    public function subscriptionsEmailVerificationServers()
    {
        return $this->hasMany('horsefly\Model\SubscriptionsEmailVerificationServer');
    }
    ...
    public function subAccount()
    {
        ...
        return $this->belongsTo('horsefly\Model\SubAccountSendGrid');
    }
    ...
    {
        return $this->getOption('sending_server_option') == \horsefly\Model\Plan::SENDING_SERVER_OPTION_SUBACCOUNT;
    }
    ...
    public static function filter($request)
    {
        ...
        if (!empty($filters)) {
            ...
            if (!empty($filters['plan_uid'])) {
                $plan = \horsefly\Model\Plan::findByUid($filters['plan_uid']);
    ...
    {
        $this->plan_id = $this->plan_id ? $this->plan_id : (!empty($params['plan_uid']) ? \horsefly\Model\Plan::findByUid($params['plan_uid'])->id : null);
        $this->customer_id = $this->customer_id ? $this->customer_id : (isset($params['customer_uid']) ? \horsefly\Model\Customer::findByUid($params['customer_uid'])->id : null);
        $this->payment_method_id = $this->payment_method_id ? $this->payment_method_id : (!empty($params['payment_method_uid']) ? \horsefly\Model\PaymentMethod::findByUid($params['payment_method_uid'])->id : null);
        ...
        if (isset($this->plan_id)) {
            ...
            switch ($plan->frequency_unit) {
                case 'day':
                    $this->end_at = $this->start_at->addDays($plan->frequency_amount);
                    break;
                case 'week':
                ...
                case 'year':
                    ...
                    break;
                case 'unlimited':
                    $this->start_at = \horsefly\Library\Tool::dateTime(\Carbon\Carbon::now());
                    $this->end_at = null;
                    break;
            }
        }
    ...
    public static function defaultOptions()
    {
        $options = [
            ...
            'sending_server_option' => \horsefly\Model\Plan::SENDING_SERVER_OPTION_SYSTEM,
    ...
    public static function defaultUnlimitedOptions()
    {
        $options = [
            ...
            'all_sending_server_types' => 'yes',
            'sending_server_types' => [],
            'sending_server_option' => \horsefly\Model\Plan::SENDING_SERVER_OPTION_OWN,
            'sending_server_subaccount_uid' => null,
            'api_access' => 'yes',
        ];
    ...
    public function displayQuota()
    {
        if ($this->getOption('sending_quota') == -1) {
            ...
            return strtolower(\horsefly\Library\Tool::format_number($this->getOption('sending_quota')).' '.trans('messages.'.\horsefly\Library\Tool::getPluralPrase('email', $this->getOption('sending_quota'))).' / '.$this->getOption('sending_quota_time').' '.trans('messages.'.\horsefly\Library\Tool::getPluralPrase($this->getOption('sending_quota_time_unit'), $this->getOption('sending_quota'))));
        }
    }
    ...
    public function displayTotalQuota()
    {
        if ($this->getOption('email_max') == -1) {
            ...
            return \horsefly\Library\Tool::format_number($this->getOption('email_max'));
        }
    }
    ...
    public function displayMaxList()
    {
        if ($this->getOption('list_max') == -1) {
            ...
            return \horsefly\Library\Tool::format_number($this->getOption('list_max'));
        }
    }
    ...
    public function displayMaxSubscriber()
    {
        if ($this->getOption('subscriber_max') == -1) {
            ...
            return \horsefly\Library\Tool::format_number($this->getOption('subscriber_max'));
        }
    }
    ...
    public function displayMaxCampaign()
    {
        if ($this->getOption('campaign_max') == -1) {
            ...
            return \horsefly\Library\Tool::format_number($this->getOption('campaign_max'));
        }
    }
    ...
    public function displayMaxSizeUploadTotal()
    {
        if ($this->getOption('max_size_upload_total') == -1) {
            ...
            return \horsefly\Library\Tool::format_number($this->getOption('max_size_upload_total'));
        }
    }
    ...
    public function displayFileSizeUpload()
    {
        if ($this->getOption('max_file_size_upload') == -1) {
            ...
            return \horsefly\Library\Tool::format_number($this->getOption('max_file_size_upload'));
        }
    }
    ...
    {
        if ($this->getOption('sending_server_option') != \horsefly\Model\Plan::SENDING_SERVER_OPTION_OWN) {
    ...
    {
        if (!$this->isStarted()) {
            return self::TIME_STATUS_FUTURE;
        } elseif ($this->isOld()) {
            return self::TIME_STATUS_PAST;
        } else {
            return self::TIME_STATUS_CURRENT;
        }
    }
    ...
    public function updateEmailVerificationServers($servers)
    {
        ...
        foreach ($servers as $key => $param) {
            if ($param['check']) {
                $server = \horsefly\Model\EmailVerificationServer::findByUid($key);
                $row = new \horsefly\Model\SubscriptionsEmailVerificationServer();
                $row->subscription_id = $this->id;
    ...
    public function setPaid($params = [])
    {
        ...
        // description
        if (!empty($params)) {
            $payment = new \horsefly\Model\Payment();
            ...
            $payment->status = \horsefly\Model\Payment::STATUS_SUCCESS;
            $payment->action = \horsefly\Model\Payment::ACTION_PAID;
    ...
    public function setUnPaid($description = '')
    {
        ...
        if (!empty($description)) {
            ...
            $payment->payment_method_id = null;
            $payment->status = \horsefly\Model\Payment::STATUS_SUCCESS;
            $payment->action = \horsefly\Model\Payment::ACTION_UNPAID;
            $payment->payment_method_name = '';
            $payment->order_id = $this->getOrderID();
            $payment->save();
        }
    ...
    {
        if ($this->isPaid()) {
            return self::PAID_STATUS_TRUE;
        } else {
            return self::PAID_STATUS_FALSE;
        }
    }
    ...
    {
        $strs = [$this->plan_name];
        $strs[] = '('.\horsefly\Library\Tool::formatDate($this->start_at);
        $strs[] = ' - '.\horsefly\Library\Tool::formatDate($this->end_at).')';
        if ($this->isCurrent()) {
    ...
    {
        return [
            ['value' => self::STATUS_ACTIVE, 'text' => trans('messages.subscription_status_'.self::STATUS_ACTIVE)],
            ['value' => self::STATUS_INACTIVE, 'text' => trans('messages.subscription_status_'.self::STATUS_INACTIVE)],
            ['value' => self::STATUS_DISABLED, 'text' => trans('messages.subscription_status_'.self::STATUS_DISABLED)],
        ];
    }
    ...
    {
        return [
            ['value' => self::TIME_STATUS_PAST, 'text' => trans('messages.subscription_time_status_'.self::TIME_STATUS_PAST)],
            ['value' => self::TIME_STATUS_CURRENT, 'text' => trans('messages.subscription_time_status_'.self::TIME_STATUS_CURRENT)],
            ['value' => self::TIME_STATUS_FUTURE, 'text' => trans('messages.subscription_time_status_'.self::TIME_STATUS_FUTURE)],
        ];
    }
    ...
    {
        return [
            ['value' => 'true', 'text' => trans('messages.subscription_paid_status_'.self::PAID_STATUS_FALSE)],
            ['value' => 1, 'text' => trans('messages.subscription_paid_status_'.self::PAID_STATUS_TRUE)],
        ];
    }
    ...
    {
        if (
            in_array($payment_method->type, [\horsefly\Model\PaymentMethod::TYPE_BRAINTREE_PAYPAL, \horsefly\Model\PaymentMethod::TYPE_BRAINTREE_CREDIT_CARD])
    ...
    public function createSubAccount()
    {
        ...
        try {
            ...
            $server = \horsefly\Model\SendingServer::findByUid($this->getOption('sending_server_subaccount_uid'));
            $account = \horsefly\Model\SubAccountSendGrid::setup([
    ...
    /**
     * Get PayPal price for countries.
     */
    public function getPayPalPrice()
    {
        ...
        if (in_array($this->currency_code, $noDecimalCodes)) {
            $price = round($this->price, 0);
        } else {
            $price = $this->price;
        }
    ...
    /**
     * Set end at datetime from start at.
     */
    public function setEndAt()
    {
        ...
        // end day
        switch ($this->plan->frequency_unit) {
            case 'day':
                $this->end_at = $this->start_at->addDays($this->plan->frequency_amount);
                break;
            case 'week':
                $this->end_at = $this->start_at->addWeeks($this->plan->frequency_amount)->subDays(1);
                break;
            case 'month':
                $this->end_at = $this->start_at->addMonths($this->plan->frequency_amount)->subDays(1);
                break;
            case 'year':
                $this->end_at = $this->start_at->addYears($this->plan->frequency_amount)->subDays(1);
                break;
            case 'unlimited':
                $this->end_at = null;
                break;
