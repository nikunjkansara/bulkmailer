...
use horsefly\Cashier\Interfaces\BillablePlanInterface;
...
class Plan extends Model implements BillablePlanInterface
{
    ...
    {
        return $this->hasMany('horsefly\Model\PlansSendingServer');
    }

    public function currency()
    {
        return $this->belongsTo('horsefly\Model\Currency');
    }

    public function subscriptions()
    {
        return $this->hasMany('horsefly\Cashier\Subscription', 'plan_id', 'uid')
    ...
    {
        return $this->hasMany('horsefly\Model\PlansEmailVerificationServer');
    }
    ...
    public static function select2($request)
    {
        ...
        $query = \horsefly\Model\Plan::getAllActive()->orderBy('custom_order', 'asc');
        if (isset($request->q)) {
            $keyword = $request->q;
            $query = $query->where(function ($q) use ($keyword) {
                $q->orwhere('plans.name', 'like', '%'.$keyword.'%');
        ...
        // Read all check
        /*if ($request->user()->admin && !$request->user()->admin->can('readAll', new \horsefly\Model\Plan())) {
            $query = $query->where('plans.admin_id', '=', $request->user()->admin->id);
        }*/

        if ($request->change_from_uid) {
            $plan = \horsefly\Model\Plan::findByUid($request->change_from_uid);
        ...
        foreach ($query->limit(20)->get() as $plan) {
            $data['items'][] = ['id' => $plan->uid, 'text' => htmlspecialchars($plan->name).'|||'.htmlspecialchars(\horsefly\Library\Tool::format_price($plan->price, $plan->currency->format))];
        }
    ...
    public static function getAllActiveWithDefault($admin = null)
    {
        ...
        if (isset($admin) && !$admin->can('readAll', new \horsefly\Model\Plan())) {
    ...
    public function displayFrequencyTime()
    {
        ...
        return $this->frequency_amount.' '.\horsefly\Library\Tool::getPluralPrase($this->frequency_unit, $this->frequency_amount);
    }
    ...
    public function fillPlansEmailVerificationServers($params)
    {
        ...
        foreach ($params as $key => $param) {
            if ($param['check']) {
                ...
                $row = new \horsefly\Model\PlansEmailVerificationServer();
                $row->plan_id = $this->id;
    ...
    public function saveAll($request)
    {
        ...
        if (config('cashier.gateway') == 'paypal_subscription' && isset($request->plan) && isset($request->plan['general'])) {
            $service = \horsefly\Cashier\Cashier::getPaymentGateway('paypal_subscription');
    ...
    {
        $planMethodlist = \horsefly\Model\PaymentMethod::getAllActive();
    ...
    public function updateFitnesses($hash)
    {
        foreach ($hash as $uid => $value) {
            ...
            \horsefly\Model\PlansSendingServer::where('sending_server_id', '=', $sendingServer->id)
    ...
    public function getVerifiedIdentities()
    {
        ...
        //
        //if (isset($server)) {
        //    return $this->primarySendingServer()->getVerifiedIdentities();
        //} else {
        //    return [];
        //}
        $result = [];
    ...
    {
        return \horsefly\Library\Tool::format_price($this->price, $this->currency->format);
    }
    ...
    public function getEmailVerificaionServers()
    {
        if ($this->getOption('all_email_verification_servers') == 'yes') {
            $result = \horsefly\Model\EmailVerificationServer::getAllAdminActive()->get()->map(function ($server) {
                return $server;
            });
        } else {
    ...
    {
        return $this->getOption('sending_server_option') == \horsefly\Model\Plan::SENDING_SERVER_OPTION_SYSTEM;
    }

    public function useOwnSendingServer()
    {
        return $this->getOption('sending_server_option') == \horsefly\Model\Plan::SENDING_SERVER_OPTION_OWN;
    }
    ...
    public static function getAvailablePlans()
    {
        ...
        $service = \horsefly\Cashier\Cashier::getPaymentGateway();
    ...
    {
        return $this->plansSendingServers()->join('sending_servers', 'sending_servers.id', 'plans_sending_servers.sending_server_id')->where('sending_servers.status', 'active');
    }
