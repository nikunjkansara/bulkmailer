...
use DB;
use horsefly\Exceptions\CampaignPausedException;
use horsefly\Exceptions\CampaignErrorException;
...
class Campaign extends AbsCampaign
{
    ...
    public function getPendingSubscribers($partition = null, $callback)
    {
        ...
        */        
        $builder = $this->subscribers([], ['email_verifications'])
                ...
                ->whereRaw(DB::getTablePrefix()."subscribers.status = '".Subscriber::STATUS_SUBSCRIBED."'")
                ->groupBy(DB::raw(table('subscribers.email')));
    ...
    {
        $this->ready();
        $job = (new \horsefly\Jobs\RunCampaignJob($this))->delay($this->getDelayInSeconds());
        dispatch($job);
    }
    ...
    public function clearAllJobs()
    {
        ...
        $systemJobs = SystemJob::where('name', 'horsefly\Jobs\RunCampaignJob')->where('data', $this->id)->get();
    ...
    {
        \horsefly\Library\Tool::xcopy($template->getStoragePath(), $this->getUploadDir());
