...
class Segment extends Model
{
    ...
    {
        return $this->hasMany('horsefly\Model\SegmentCondition');
    }
    ...
    public function getSubscribersConditions()
    {
        ...
        foreach ($this->segmentConditions as $index => $condition) {
            ...
            if (isset($condition->field_id)) {
                ...
                // add condition
                $conditions[] = $cond;
            } elseif (strpos($condition->operator, 'verification') === 0) {
                switch ($condition->operator) {
            ...
            } elseif (strpos($condition->operator, 'tag') === 0) {
                switch ($condition->operator) {
                    case 'tag_contains':
                        // add condition
    ...
    {
        $query = \horsefly\Model\Subscriber::select('subscribers.*', 'email_verifications.result AS verify_result');
    ...
    public function updateCacheDelayed()
    {
        $existed = SystemJobModel::getNewJobs()
                       ->where('name', \horsefly\Jobs\UpdateSegmentJob::class)
                       ->where('data', $this->id)
                       ->exists();
        ...
        if (!$existed) {
            dispatch(new \horsefly\Jobs\UpdateSegmentJob($this));
        }
    }
