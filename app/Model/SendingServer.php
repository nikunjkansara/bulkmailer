...
use horsefly\Library\Notification\BackendError as BackendErrorNotification;
...
class SendingServer extends Model
{
    ...
    {
        return $this->hasMany('horsefly\Model\TrackingLog', 'sending_server_id')->orderBy('created_at', 'asc');
    }
    ...
    {
        return $this->belongsToMany('horsefly\Model\Plan', 'plans_sending_servers');
    }
    ...
    {
        return $this->belongsTo('horsefly\Model\BounceHandler');
    }
    ...
    {
        $class_name = '\horsefly\Model\\'.self::$serverMapping[$server->type];
    ...
    public function displayQuota()
    {
        ...
        return $this->quota_value.'/'.$this->quota_base.' '.trans('messages.'.\horsefly\Library\Tool::getPluralPrase($this->quota_unit, $this->quota_base));
    }
    ...
    public function displayQuotaHtml()
    {
        ...
        return '<b>'.$this->quota_value.'</b>/<b>'.$this->quota_base.' '.trans('messages.'.\horsefly\Library\Tool::getPluralPrase($this->quota_unit, $this->quota_base)).'</b>';
    }
    ...
    public static function select2($request)
    {
        ...
        // plan
        if ($request->plan_uid) {
            $plan = \horsefly\Model\Plan::findByUid($request->plan_uid);
    ...
    public function getSendingLimitSelectOptions()
    {
        ...
        $current = trans('messages.sending_servers.sending_limit.phrase', [
            'quota_value' => \horsefly\Library\Tool::format_number($this->quota_value),
            'quota_base' => \horsefly\Library\Tool::format_number($this->quota_base),
        ...
        foreach (self::sendingLimitValues() as $key => $data) {
            $wording = trans('messages.sending_servers.sending_limit.phrase', [
                'quota_value' => \horsefly\Library\Tool::format_number($data['quota_value']),
                'quota_base' => \horsefly\Library\Tool::format_number($data['quota_base']),
    ...
    public function addDomain($domain)
    {
        ...
        $this->setOption('domains', $domains);
        $this->save();
    }
    ...
    {
        return array_merge($this->getOption('domains'), $this->getOption('emails'));
    }
     ...
     * @return array
     */
    public function initVerifiedIdentities()
    {
        $items = $this->getVerifiedIdentities();
