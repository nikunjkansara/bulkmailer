...
use DB;
use horsefly\Library\Automation\Action;
use horsefly\Library\Automation\Trigger;
use horsefly\Library\Automation\Send;
use horsefly\Library\Automation\Wait;
use horsefly\Library\Automation\Evaluate;
use horsefly\Library\Automation\Operate;
use Carbon\Carbon;
use Monolog\Logger;
...
class Automation2 extends Model
{
    ...
    {
        return $this->hasManyThrough('horsefly\Model\EmailLink', 'horsefly\Model\Email');
    }
    ...
    {
        return $this->hasMany('horsefly\Model\Email');
    }
    ...
    {
        return $this->hasMany('horsefly\Model\AutoTrigger');
    }
    ...
    {
        return $this->hasMany('horsefly\Model\Timeline')->orderBy('created_at', 'DESC');
    }
    ...
    public function fillRequest($request)
    {
        ...
        if (!empty($request->segment_uid)) {
            foreach ($request->segment_uid as $segmentUid) {
                $segments[] = \horsefly\Model\Segment::findByUid($segmentUid)->id;
            }
    ...
    public function getSegments()
    {
        ...
        $segments = \horsefly\Model\Segment::whereIn('id', explode(',', $this->segment_id))->get();
