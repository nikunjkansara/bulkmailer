...
abstract class AbsCampaign extends Model
{
    ...
    {
        return $this->belongsTo('horsefly\Model\MailList', 'default_mail_list_id');
    }
    ...
    {
        return $this->belongsToMany('horsefly\Model\MailList', 'campaigns_lists_segments');
    }
    ...
    {
        return $this->belongsTo('horsefly\Model\SendingDomain', 'tracking_domain_id');
    }
    ...
    {
        return $this->belongsToMany('horsefly\Model\Link', 'campaign_links');
    }
    ...
    {
        return $this->hasMany('horsefly\Model\TrackingLog');
    }
    ...
    {
        return $this->hasMany('horsefly\Model\CampaignsListsSegment');
    }
    ...
    public function getListsSegmentsGroups()
    {
        ...
        foreach ($lists_segments as $lists_segment) {
            if (!isset($groups[$lists_segment->mail_list_id])) {
                ...
                if(!isset($this->default_mail_list_id))
                    $groups[$lists_segment->mail_list_id]['is_default'] = true;
    ...
    public function appendOpenTrackingUrl($body, $msgId)
    {
        ...
        MailLog::info("Mail Track Open <img src='{$url}' />");
    ...
    public function getHtmlContent($subscriber, $msgId, $server = null)
    {
        ...
        if ($this->track_click) {
            MailLog::info("Mail Track Click `{$msgId}`");
            $body = $this->addTrackingUrl($body, $msgId);
        }

        // Enable open tracking
        if ($this->track_open) {
            MailLog::info("Mail Track Open `{$msgId}`");
    ...
    public function getPlainContent($subscriber, $msgId, $server = null)
    {
        ...
        // Append footer
        if ($this->footerEnabled()) {
            $body = $this->appendFooter($plain, $this->getPlainTextFooter());
        }
    ...
    public static function boot()
    {
        ...
        static::deleted(function ($item) {
            ...
            \horsefly\Library\Tool::xdelete($uploaded_dir);
        });
    }
    ...
    {
        return $this->subscribersCount() - $this->trackingLogs()->count();
    }
    ...
    {
        return $this->unsubscribeLogs()->distinct('unsubscribe_logs.subscriber_id')->count('unsubscribe_logs.subscriber_id');
    }
    ...
    {
        return \horsefly\Web\Subscriber::selectRaw(DB::getTablePrefix().'list_subscriber.*, COUNT('.DB::getTablePrefix().'campaign_track_unsubscribe.id) AS openCount')
    ...
    {
        // clean up failed log so that they will be included in resend
        $this->trackingLogs()->where('status', SendingServer::DELIVERY_STATUS_FAILED)->delete();
    }
    ...
    public function openUniqHours($start = null, $end = null)
    {
        ...
        return $query->orderBy('open_logs.created_at', 'asc')->get()->groupBy(function ($date) {
            return \horsefly\Library\Tool::dateTime($date->created_at)->format('H'); // grouping by hours
        });
    }
    ...
    public function fillAttributes($params)
    {
        ...
        // Tacking domain
        if (isset($params['tracking_domain_uid'])) {
            $tracking_domain = \horsefly\Model\SendingDomain::findByUid($params['tracking_domain_uid']);
    ...
    public function transform()
    {
        ...
        $this->html = '<!DOCTYPE html>'.\horsefly\Library\Tool::replaceTemplateUrl($this->html, route('campaign_assets', ['uid' => $this->uid, 'path' => '']));
    ...
    public function untransform()
    {
        ...
        $this->html = str_replace(route('campaign_assets', ['uid' => $this->uid, 'path' => '']).'/', '', $this->html);
    }
    ...
    public function copyFromTemplate($template)
    {
        ...
        \horsefly\Library\Tool::xcopy($template->getStoragePath(), $this->getStoragePath());
     ...
     * Copy new template.
     */
    public function generateTrackingLogCsv($progressCallback = null)
    {
        ...
        DB::statement('CREATE TEMPORARY TABLE '.table($tmpTableName)." AS SELECT
            ...
            IF(o.id IS NULL, 'no', 'yes') as `open`,
            IF(ck.id IS NULL, 'no', 'yes') as `click`,
            IF(u.id IS NULL, 'no', 'yes') as `unsubscribe`,
            IF(f.id IS NULL, 'no', 'yes') as `feedback`,
            IF(b.id IS NULL, 'no', 'yes') as `bounce`,
            o.created_at AS open_at,
            ck.created_at AS click_at,
            ck.url as click_url,
            ck.ip_address as click_ip_address,
            ck.user_agent as click_user_agent,
          ...
          LEFT JOIN '.table('open_logs').' o ON o.message_id = l.message_id
          LEFT JOIN '.table('click_logs').' ck ON ck.message_id = l.message_id
          LEFT JOIN '.table('unsubscribe_logs').' u ON u.message_id = l.message_id
          LEFT JOIN '.table('feedback_logs').' f ON f.message_id = l.message_id
          LEFT JOIN '.table('bounce_logs').' b ON b.message_id = l.message_id');
