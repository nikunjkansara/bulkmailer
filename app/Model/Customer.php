...
use horsefly\Library\QuotaTrackerFile;
use Illuminate\Database\Eloquent\Model;
use Carbon\Carbon;
use horsefly\Cashier\Interfaces\BillableUserInterface;
...
class Customer extends Model implements BillableUserInterface
{
    ...
    {
        return $this->hasOne('horsefly\Cashier\Subscription', 'user_id', 'uid')
    ...
    {
        return $this->belongsTo('horsefly\Model\Contact');
    }
    ...
    {
        return $this->belongsTo('horsefly\Model\Admin');
    }

    public function lists()
    {
        return $this->hasMany('horsefly\Model\MailList')->orderBy('created_at', 'desc');
    }

    public function language()
    {
        return $this->belongsTo('horsefly\Model\Language');
    }

    public function campaigns()
    {
        return $this->hasMany('horsefly\Model\Campaign')->orderBy('created_at', 'desc');
    }

    public function sentCampaigns()
    {
        return $this->hasMany('horsefly\Model\Campaign')->where('status', '=', 'done')->orderBy('created_at', 'desc');
    }

    public function subscribers()
    {
        return $this->hasManyThrough('horsefly\Model\Subscriber', 'horsefly\Model\MailList');
    }

    public function logs()
    {
        return $this->hasMany('horsefly\Model\Log')->orderBy('created_at', 'desc');
    }

    public function trackingLogs()
    {
        return $this->hasMany('horsefly\Model\TrackingLog')->orderBy('created_at', 'asc');
    }

    public function automation2s()
    {
        return $this->hasMany('horsefly\Model\Automation2');
    }

    public function activeAutomation2s()
    {
        return $this->hasMany('horsefly\Model\Automation2')->where('status', Automation2::STATUS_ACTIVE);
    }

    public function sendingDomains()
    {
        return $this->hasMany('horsefly\Model\SendingDomain');
    }
    ...
    {
        return $this->hasMany('horsefly\Model\EmailVerificationServer');
    }
    ...
    {
        return $this->hasMany('horsefly\Model\Blacklist');
    }

    public function senders()
    {
        return $this->hasMany('horsefly\Model\Sender')->where('status', '=', \horsefly\Model\Sender::STATUS_VERIFIED);
    }

    public function activeVerifiedSendingDomains()
    {
        return $this->sendingDomains()
            ->where('status', '=', \horsefly\Model\SendingDomain::STATUS_ACTIVE)
            ->where('domain_verified', '=', \horsefly\Model\SendingDomain::VERIFIED);
    }
    ...
    {
        $query = \horsefly\Model\Subscriber::leftJoin('mail_lists', 'mail_lists.id', '=', 'subscribers.mail_list_id')
    ...
    public function getColorScheme()
    {
        if (!empty($this->color_scheme)) {
            ...
            return \horsefly\Model\Setting::get('frontend_scheme');
        }
    }
    ...
    public static function select2($request)
    {
        ...
        $query = \horsefly\Model\Customer::getAll()->leftJoin('users', 'users.id', '=', 'customers.user_id');
        if (isset($request->q)) {
            $keyword = $request->q;
            $query = $query->where(function ($q) use ($keyword) {
                $q->orwhere('customers.first_name', 'like', '%'.$keyword.'%')
        ...
        // Read all check
        if (!$request->user()->admin->can('readAll', new \horsefly\Model\Customer())) {
    ...
    public function updateInformation($request)
    {
        ...
        $user = is_object($this->user) ? $this->user : new \horsefly\Model\User();
    ...
    {
        return $this->hasMany('horsefly\Model\SendingServer');
    }

    public function subAccounts()
    {
        return $this->hasMany('horsefly\Model\SubAccount');
    }
    ...
    {
        $query = \horsefly\Model\Customer::select('customers.*');

        if (isset($admin) && !$admin->can('readAll', new \horsefly\Model\Customer())) {
    ...
    {
        return $this->sendingServers()->where('status', '=', \horsefly\Model\SendingServer::STATUS_ACTIVE);
    }
    ...
    {
        return \horsefly\Library\Tool::getDirectorySize(base_path('public/source/'.$this->user->uid)) / 1048576;
    }
    ...
    public function getSendingDomains()
    {
        ...
        // If customer dont have permission creating sending domain
            ...
            $result = \horsefly\Model\SendingDomain::getAllAdminActive()->get()->map(function ($server) {
                return $server;
            });
        }
    ...
    {
        $query = \horsefly\Model\MailList::getAll();
    ...
    {
        $allTypes = \horsefly\Model\SendingServer::types();
        $types = [];
    ...
    {
        return \horsefly\Model\SystemJob::where('name', '=', "horsefly\Jobs\ImportBlacklistJob")
    ...
    public function getActiveImportBlacklistJobs()
    {
        return $this->getImportBlacklistJobs()
            ->where('status', '!=', \horsefly\Model\SystemJob::STATUS_DONE)
            ->where('status', '!=', \horsefly\Model\SystemJob::STATUS_FAILED)
            ->where('status', '!=', \horsefly\Model\SystemJob::STATUS_CANCELLED);
    }
    ...
    public function addEmaillToBlacklist($email)
    {
        ...
        if (\horsefly\Library\Tool::isValidEmail($email)) {
            $exist = $this->blacklists()->where('email', '=', $email)->count();
            if (!$exist) {
                $blacklist = new \horsefly\Model\Blacklist();
    ...
    {
        return \horsefly\Model\SendingDomain::getAllActive();
    }
    ...
    {
        $gateway = \horsefly\Cashier\Cashier::getPaymentGateway();

        if (is_object($this->subscription)) {
            if ($this->subscription->isNew() || $this->subscription->isPending()) {
                return true;
            }

            if ($gateway->hasPending($this->subscription) || $gateway->hasError($this->subscription)) {
                return true;
            }
        }
