...
use horsefly\Library\RouletteWheel;
use horsefly\Library\StringHelper;
use horsefly\Exceptions\VerificationProcessCancelledException;
...
class MailList extends Model
{
    ...
    {
        return $this->hasMany('horsefly\Model\Field');
    }
    ...
    {
        return $this->hasMany('horsefly\Model\Segment');
    }
    ...
    {
        return $this->hasMany('horsefly\Model\Page');
    }
    ...
    {
        return $this->hasMany('horsefly\Model\Subscriber');
    }

    public function campaigns()
    {
        return $this->belongsToMany('horsefly\Model\Campaign', 'campaigns_lists_segments', 'mail_list_id', 'campaign_id');
    }
    ...
    public function createDefaultFieds()
    {
        ...
        $this->fields()->create([
                            ...
                            'tag' => \horsefly\Model\Field::formatTag(trans('messages.first_name_tag')),
                            'required' => false,
                            'visible' => true,
                        ]);
        ...
        $this->fields()->create([
                            ...
                            'tag' => \horsefly\Model\Field::formatTag(trans('messages.last_name_tag')),
                            'required' => false,
                            'visible' => true,
                        ]);
    }
    ...
    public function longName($cache = false)
    {
        ...
        return $this->name.' - '.$count.' '.trans('messages.'.\horsefly\Library\Tool::getPluralPrase('subscriber', $count)).'';
    }
    ...
    {
        return \horsefly\Model\SystemJob::where('name', '=', "horsefly\Jobs\ImportSubscribersJob")
    ...
    {
        return \horsefly\Model\SystemJob::where('name', '=', "horsefly\Jobs\ExportSubscribersJob")
            ->where('data', 'like', '%"mail_list_uid":"'.$this->uid.'"%');
    }
    ...
    {
        return \horsefly\Model\SystemJob::where('name', '=', "horsefly\Jobs\ExportSegmentsJob")
    ...
    public static function export($list, $customer, $job)
    {
        ...
        $lines_per_second = 1;
        $headers = [];
        $headers[] = trans('messages.list.exported_file.headers.uid');
        $headers[] = trans('messages.list.exported_file.headers.status');
        ...
        for ($page = 0; $page <= ceil($total / $num); ++$page) { // ceil($total/$num)
            ...
            foreach ($list->subscribers()->skip($page * $num)->take($num)->get() as $key => $item) {
                ...
                $data[] = \horsefly\Library\Tool::arrayToCsv($cols, ',');
    ...
    public static function exportSegments($list, $customer, $job)
    {
        ...
        $headers = $json->fields;
        $segment = \horsefly\Model\Segment::findByUid($json->uid);
        ...
        for ($page = 0; $page <= ceil($total / $num); ++$page) { // ceil($total/$num)
            ...
            $content_cache = trans('messages.export_segments_statistics_line', [
                'total' => $total,
                'processed' => $success + $error,
                'success' => $success,
                'error' => $error,
    ...
    public function sendSubscriptionConfirmationEmail($subscriber)
    {
        ...
        $layout = \horsefly\Model\Layout::where('alias', 'sign_up_confirmation_email')->first();
        $send_page = \horsefly\Model\Page::findPage($list, $layout);
    ...
    public function sendSubscriptionWelcomeEmail($subscriber)
    {
        ...
        $layout = \horsefly\Model\Layout::where('alias', 'sign_up_welcome_email')->first();
        $send_page = \horsefly\Model\Page::findPage($list, $layout);
        $this->sendMail($subscriber, $send_page, $send_page->getTransformedSubject($subscriber));
    }
    ...
    public function sendUnsubscriptionNotificationEmail($subscriber)
    {
        ...
        $layout = \horsefly\Model\Layout::where('alias', 'unsubscribe_goodbye_email')->first();
        $send_page = \horsefly\Model\Page::findPage($list, $layout);
        $this->sendMail($subscriber, $send_page, $send_page->getTransformedSubject($subscriber));
    }
    ...
    public function sendProfileUpdateEmail($subscriber)
    {
        ...
        $layout = \horsefly\Model\Layout::where('alias', 'profile_update_email')->first();
        $send_page = \horsefly\Model\Page::findPage($list, $layout);
        $this->sendMail($subscriber, $send_page, $send_page->getTransformedSubject($subscriber));
    }
    ...
    private function readCsv($file)
    {
        try {
            ...
            $encoding = \horsefly\Library\StringHelper::detectEncoding($file);

            if ($encoding == false) {
                MailLog::warning("Cannot detect file's encoding: {$file}");
            } elseif ($encoding != 'UTF-8') {
                MailLog::warning("Convert from {$encoding} to UTF-8");
                \horsefly\Library\StringHelper::toUTF8($file, $encoding);
            } else {
                MailLog::info('File encoding is UTF-8');
                \horsefly\Library\StringHelper::checkAndRemoveUTF8BOM($file);
            }
    ...
    {
        return $this->hasMany('horsefly\Model\MailListsSendingServer');
    }
    ...
    public function sendMail($subscriber, $page, $title)
    {
        ...
        try {
            ...
            if ($result['status'] == SendingServer::DELIVERY_STATUS_FAILED) {
                throw new \Exception('Error returned from sending service: '.$result['error']);
            }
        } catch (\Exception $ex) {
            $error = $ex->getMessage();
            MailLog::error($error);
    ...
    public function getSendingServers()
    {
        ...
        if ($this->customer->getOption('sending_server_option') == \horsefly\Model\Plan::SENDING_SERVER_OPTION_OWN) {
        ...
        } elseif ($this->customer->getOption('sending_server_option') == \horsefly\Model\Plan::SENDING_SERVER_OPTION_SYSTEM) {
            // Check if has sending servers for current subscription
            if (is_object($subscription)) {
                /*
                if ($subscription->plan->getOption('all_sending_servers') == 'yes') {
                    if (\horsefly\Model\SendingServer::getAllAdminActive()->count()) {
                        $result = \horsefly\Model\SendingServer::getAllAdminActive()->get()->map(function ($server) {
                            return [$server->id, '100'];
                        });
                    }
    ...
    public function queueForVerification($serverId)
    {
        ...
        if (is_null($job)) {
            $job = (new \horsefly\Jobs\VerifyMailListJob($this->id, $serverId));
            dispatch($job);
        } else {
    ...
    {
        $job = SystemJob::where('name', 'horsefly\Jobs\VerifyMailListJob')
    ...
    /**
     * Get one field for list.
     */
    public function getOneField()
    {
        return $this->getFields()->where('tag', '<>', 'EMAIL')->first();
    }
}
