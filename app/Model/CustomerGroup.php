...
class CustomerGroup extends Model
{
    ...
    {
        return $this->hasMany('horsefly\Model\CustomerGroupSendingServer');
    }

    public function sending_servers()
    {
        return $this->belongsToMany('horsefly\Model\SendingServer', 'customer_group_sending_servers');
    }
    ...
    public static function boot()
    {
        ...
        static::creating(function ($item) {
            // Update custom order
            CustomerGroup::getAll()->increment('custom_order', 1);
            $item->custom_order = 0;
        });
    }
    ...
    public function displayQuota()
    {
        if ($this->getOption('sending_quota') == -1) {
            ...
            return $this->getOption('sending_quota').' '.trans('messages.'.\horsefly\Library\Tool::getPluralPrase('email', $this->getOption('sending_quota'))).' / '.$this->getOption('sending_quota_time').' '.trans('messages.'.\horsefly\Library\Tool::getPluralPrase($this->getOption('sending_quota_time_unit'), $this->getOption('sending_quota')));
        }
    }
