	...
	<div class="navbar navbar-{{ Auth::user()->admin->getColorScheme() == "white" ? "default" : "inverse" }} navbar-fixed-top">
		...
		<div class="navbar-collapse collapse" id="navbar-mobile">
			<ul class="nav navbar-nav">
				...
				@if (Auth::user()->can("read", new horsefly\Model\Customer()))
				...
				@if (
					Auth::user()->can("read", new horsefly\Model\Plan())
					|| Auth::user()->can("read", new horsefly\Model\Currency())
				)
					<li class="dropdown language-switch"
						rel0="PlanController"
					...
					>
						...
						<ul class="dropdown-menu">
							@if (Auth::user()->can("read", new horsefly\Model\Plan()))
							...
							@endif
							@if (Auth::user()->can("read", new horsefly\Model\Currency()))
				...
				@if (
					...
					|| Auth::user()->admin->can('read', new \horsefly\Model\SubAccount())
				)
					<li class="dropdown language-switch"
					...
					>
						...
						<ul class="dropdown-menu">
							...
							@endif
							@if (Auth::user()->admin->can('read', new \horsefly\Model\SubAccount()))
                    ...
                    <ul class="dropdown-menu">
						...
						<li rel0="PaymentController">
							<a href="{{ action('Admin\PaymentController@index') }}">
								<i class="icon-credit-card2"></i> {{ trans('messages.payment_gateways') }}
							</a>
						</li>
                    </ul>
				</li>
				...
				<li class="dropdown dropdown-user">
					...
					<ul class="dropdown-menu dropdown-menu-right">
						@can("customer_access", Auth::user())
							<li><a href="{{ route("dashboard") }}"><i class="icon-exit2"></i> {{ trans('messages.customer_view') }}</a></li>
							<li class="divider"></li>
						@endif
