    <!-- Basic modal -->
	<div id="confirm-modal" class="modal fade mc-modal">
			...
			<div class="modal-content">
				<form class="confirm-link-form" onkeypress="return event.keyCode != 13;">
					...
					<div class="modal-footer">
						<button type="button" class="btn btn-mc_inline" data-dismiss="modal">{{ trans('messages.cancel') }}</button>
						<a class="btn btn-mc_primary link-confirm-button">{{ trans('messages.ok') }}</a>
					</div>
				</form>
			</div>
		</div>
		...
		<div class="modal-dialog">
			<div class="modal-content">
				<form class="confirm-link-form" onkeypress="return event.keyCode != 13;">
					...
					<div class="modal-footer">
						<button type="button" class="btn btn-mc_inline" data-dismiss="modal">{{ trans('messages.cancel') }}</button>
						<a class="btn btn-mc_primary link-confirm-button ajax_link">{{ trans('messages.confirm') }}</a>
					</div>
				</form>
			</div>
		</div>
				...
				<form action="{{ action('MailListController@copy') }}" method="POST" class="ajax_copy_list_form form-validate-jqueryz">
					...
					<div class="modal-body">
						...
						@include('helpers.form_control', [
							...
							'options' => horsefly\Model\CustomerGroup::timeUnitOptions(),
	...
	<div class="ui-pnotify alert-warning mc-notify-wanring" style="width: auto; right: 20px; top: auto; bottom: 20px; opacity: 1; display: block; overflow: visible; cursor: auto;">
		<div class="btn-close-pnotify">{{ trans('messages.notify.clear_all') }}</div>
		@if (null !== Session::get('orig_admin_id') && Auth::user()->admin)
			<div class="alert ui-pnotify-container alert-warning ui-pnotify-shadow" style="min-height: 16px; overflow: hidden;">
		...
		@if (\horsefly\Library\Tool::currentView() == 'frontend')
			<!-- Alert if customer use their own sending servers but haven't had any one yet -->
			@if (
				is_object(\Auth::user()->customer) &&
				\Auth::user()->customer->getOption("sending_server_option") == \horsefly\Model\Plan::SENDING_SERVER_OPTION_OWN &&
		...
		@if (\horsefly\Library\Tool::currentView() == 'backend')
			<!-- Alert if system haven't had any sending servers -->
			@if (
				!\horsefly\Model\SendingServer::getAllAdminActive()->count()
			)
				<div class="alert ui-pnotify-container alert-warning ui-pnotify-shadow" style="min-height: 16px; overflow: hidden;">
					<h4 class="ui-pnotify-title">
				...
				<form action="{{ action('CampaignController@copy') }}" method="POST" class="ajax_copy_campaign_form form-validate-jquery">
					...
					<div class="modal-body">
						...
						@include('helpers.form_control', [
							...
							'options' => horsefly\Library\Tool::timeUnitOptions(),
							'include_blank' => trans('messages.choose'),
							'help_class' => 'campaign',
  ...
  <a style="display: none" type="button" class="btn btn-default btn-sm" data-toggle="modal" data-target="#copy_plan"></a>
  <!-- Basic modal -->
  <div id="copy_plan" class="modal fade">
    <div class="modal-dialog">
      <div class="modal-content">
        <div class="modal-header bg-teal">
          <button type="button" class="close" data-dismiss="modal">&times;</button>
          <h4 class="modal-title">{{ trans('messages.name_new_plan') }}</h4>
        </div>
        <form action="{{ action('Admin\PlanController@copy') }}" method="POST" class="ajax_copy_plan_form form-validate-jquery">
          {{ csrf_field() }}
          <input type="hidden" name="copy_plan_uid" />
          ...
          <div class="modal-body">
            ...
            @include('helpers.form_control', [
              ...
              'name' => 'copy_plan_name',
              'value' => '',
              'label' => trans('messages.what_would_you_like_to_name_your_plan'),
              'options' => horsefly\Library\Tool::timeUnitOptions(),
              'include_blank' => trans('messages.choose'),
              'help_class' => 'campaign',
              'rules' => ['copy_plan_name' => 'required']
            ])


            <div class="text-right">
              <button type="submit" class="btn btn-info bg-teal-600 mr-5">{{ trans('messages.copy') }}</button>
              <button type="button" class="btn btn-default ml-0 copy-plan-close" data-dismiss="modal">{{ trans('messages.close') }}</button>
            </div>
