    ...
    <div class="row quota_box">
        ...
        <div class="col-sm-6 col-md-6">
            <div class="content-group-sm">
                ...
                <div class="progress progress-sm">
                    <div class="progress-bar bg-{{ Auth::user()->customer->listsUsage() >= 80 ? 'danger' : 'primary' }}" style="width: {{ Auth::user()->customer->listsUsage() }}%">
                    </div>
                </div>
            </div>
        </div>
        <div class="col-sm-6 col-md-6">
            <div class="content-group-sm">
                <div class="pull-right  text-semibold">
                    <span class="text-muted">{{ \horsefly\Library\Tool::format_number(Auth::user()->customer->campaignsCount()) }}/{{ \horsefly\Library\Tool::format_number(Auth::user()->customer->maxCampaigns()) }}</span>
                    &nbsp;&nbsp;&nbsp;{{ Auth::user()->customer->displayCampaignsUsage() }}
                </div>
                <h5 class="text-semibold mb-5 mt-20">{{ trans('messages.campaign') }}</h5>
                <div class="progress progress-sm">
                    <div class="progress-bar bg-{{ Auth::user()->customer->campaignsUsage() >= 80 ? 'danger' : 'primary' }}" style="width: {{ Auth::user()->customer->campaignsUsage() }}%">
                    </div>
                </div>
            </div>
        </div>
        <div class="col-sm-6 col-md-6">
            <div class="content-group-sm">
                <div class="pull-right  text-semibold">
                    <span class="text-muted">{{ \horsefly\Library\Tool::format_number(Auth::user()->customer->readCache('SubscriberCount', 0)) }}/{{ \horsefly\Library\Tool::format_number(Auth::user()->customer->maxSubscribers()) }}</span>
                    &nbsp;&nbsp;&nbsp;{{ Auth::user()->customer->displaySubscribersUsage() }}
                </div>
                <h5 class="text-semibold mb-5 mt-20">{{ trans('messages.subscriber') }}</h5>
                <div class="progress progress-sm">
                    <div class="progress-bar bg-{{ Auth::user()->customer->subscribersUsage() >= 80 ? 'danger' : 'primary' }}" style="width: {{ Auth::user()->customer->readCache('SubscriberUsage', 0) }}%">
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="row">
            <div class="col-md-6">
                @include('helpers.form_control', [
                    ...
                    'label' => '',
                    'value' => '',
                    'options' => horsefly\Model\Campaign::getSelectOptions(Auth::user()->customer, "done"),
                ])
            </div>
        </div>
        ...
        <div class="tab-content">
            <div class="tab-pane active" id="top-tab1">
                <ul class="modern-listing mt-0 top-border-none">
                    @forelse (horsefly\Model\Campaign::topOpens(5, Auth::user()->customer)->get() as $num => $item)
                        <li>
                            <div class="row">
                                ...
                                <div class="col-sm-2 col-md-2 text-left">
                                    <h5 class="no-margin text-bold">
                                        {{ (null !== $item->lastOpen()) ? horsefly\Library\Tool::formatDateTime($item->lastOpen()->created_at) : "" }}
                                    </h5>
            ...
            <div class="tab-pane" id="top-tab2">
                <ul class="modern-listing mt-0 top-border-none">
                    @forelse (horsefly\Model\Campaign::topClicks(5, Auth::user()->customer)->get() as $num => $item)
                        <li>
                            <div class="row">
                                <div class="col-sm-5 col-md-5">
                                    <i class="number">{{ $num+1 }}</i>
                                ...
                                <div class="col-sm-2 col-md-2 text-left">
                                    <h5 class="no-margin text-bold">
                                        {{ (null !== $item->lastClick()) ? horsefly\Library\Tool::formatDateTime($item->lastClick()->created_at) : "" }}
                                    </h5>
            ...
            <div class="tab-pane" id="top-tab3">
                <ul class="modern-listing mt-0 top-border-none">
                    @forelse (horsefly\Model\Campaign::topLinks(5, Auth::user()->customer)->get() as $num => $item)
                        <li>
                            <div class="row">
                                <div class="col-sm-6 col-md-6">
                                    <i class="number">{{ $num+1 }}</i>
                                ...
                                <div class="col-sm-2 col-md-2 text-left">
                                    <h5 class="no-margin text-bold">
                                        {{ (null !== $item->lastClick(Auth::user()->customer)) ? horsefly\Library\Tool::formatDateTime($item->lastClick(Auth::user()->customer)->created_at) : "" }}
                                    </h5>
                                    <span class="text-muted">{{ trans('messages.last_clicked') }}</span>
                                    <br /><br />
                                </div>
