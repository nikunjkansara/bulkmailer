...
<h2 class="text-semibold text-teal-800">{{ trans('messages.feedback_log') }}</h2>

<form class="listing-form"
        data-url="{{ action('CampaignController@feedbackLogListing', $campaign->uid) }}"
        per-page="{{ horsefly\Model\FeedbackLog::$itemsPerPage }}"
    >
        <div class="row top-list-controls">
            ...
            <div class="col-md-2 text-right">
                <a href="javascript:goToDownLoad()" type="button" class="btn bg-info-800">
                    <i class="icon icon-download mr-5"></i> {{ trans('messages.feedback_log.download') }}
                </a>
            </div>
        </div>
