...
@section('content')
    ...
    <h2 class="text-semibold text-teal-800">{{ trans('messages.bounce_log') }}</h2>

    <form class="listing-form"
        data-url="{{ action('CampaignController@bounceLogListing', $campaign->uid) }}"
        per-page="{{ horsefly\Model\BounceLog::$itemsPerPage }}"
    >
        <div class="row top-list-controls">
            ...
            <div class="col-md-2 text-right">
                <a href="javascript:goToDownLoad()" type="button" class="btn bg-info-800">
                    <i class="icon icon-download mr-5"></i> {{ trans('messages.bounce_log.download') }}
                </a>
            </div>
        </div>
