...
@section('page_header')
    ...
    <div class="page-title">
        ...
        <h1>
            <span class="text-semibold"><i class="icon-list2"></i> {{ trans('messages.admin_groups') }}</span>
        </h1>
    </div>
...
@section('content')
    ...
    <form class="listing-form"
        sort-url="{{ action('Admin\AdminGroupController@sort') }}"
        data-url="{{ action('Admin\AdminGroupController@listing') }}"
        per-page="{{ horsefly\Model\AdminGroup::$itemsPerPage }}"
    >
        <div class="row top-list-controls">
            ...
            @can('create', new horsefly\Model\AdminGroup())
                <div class="col-md-2 text-right">
                    <a href="{{ action("Admin\AdminGroupController@create") }}" type="button" class="btn bg-info-800">
                        <i class="icon icon-plus2"></i> {{ trans('messages.create_admin_group') }}
                    </a>
                </div>
            @endcan
        </div>
