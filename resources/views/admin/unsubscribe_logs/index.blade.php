...
@section('title', trans('messages.unsubscribe_log'))
...
@section('page_header')
			...
			<div class="page-title">				
				...
				<h1>
					<span class="text-semibold"><i class="icon-list2"></i> {{ trans('messages.unsubscribe_log') }}</span>
				</h1>				
			</div>
...
@section('content')
				...
				<form class="listing-form"
					data-url="{{ action('Admin\UnsubscribeLogController@listing') }}"
					per-page="{{ horsefly\Model\UnsubscribeLog::$itemsPerPage }}"					
				>				
								...
								<div class="filter-box">
									<span class="filter-group">
										...
										<select class="select" name="sort-order">
                                            <option value="unsubscribe_logs.created_at">{{ trans('messages.created_at') }}</option>
										</select>										
