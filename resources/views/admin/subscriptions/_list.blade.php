	...
	>
		@foreach ($subscriptions as $key => $subscription)
			<tr>
				...
				<td width="15%">
                    <h5 class="no-margin">
                        <span class="kq_search">{{ horsefly\Library\Tool::dateTime($subscription->created_at)->format('M d, Y') }}</span>
                    </h5>
				...
				<td width="15%">
					@if ($subscription->isEnded())
						<h5 class="no-margin">
								<span class="kq_search">{{ horsefly\Library\Tool::formatDate($subscription->ends_at) }}</span>
							</h5>
						<span class="text-muted2">{{ trans('messages.subscription.subscription_ended_at') }}</span>
					@elseif ($subscription->cancelled())
						<h5 class="no-margin">
							@if ($subscription->current_period_ends_at)
								<span class="kq_search">{{ horsefly\Library\Tool::dateTime($subscription->current_period_ends_at)->diffForHumans() }}</span>
							@else
					...
					@elseif ($subscription->isRecurring())
						<h5 class="no-margin">
							<span class="kq_search">
								@if ($subscription->current_period_ends_at)
									{{ horsefly\Library\Tool::dateTime($subscription->current_period_ends_at)->diffForHumans() }}
								@else
									--
								@endif									
							</span>
