                        ...
                        @if ($contact->getTags())
                            @foreach ($contact->getTags() as $tag)
                                <a href="{{ action('Automation2Controller@removeTag', [
                                    ...
                                    <button type="button" class="btn btn-light btn-tag">{{ $tag }}</button>
                                    <button type="button" class="btn btn-light btn-tag">
                    ...
                    @foreach($automation->timelinesBy($contact)->get() as $timeline)
                        <div class="activity d-flex py-3 px-4">
                            <div class="activity-media pr-4 text-center">
                                ...
                                {!! horsefly\Model\AutomationElement::getIconByType($timeline->activity_type) !!}
                            </div>
                            <div class="small">
                                ...
                                <desc class="d-block small text-muted">
                                    <i class="lnr lnr-clock mr-1"></i> {{ horsefly\Library\Tool::formatDateTime($timeline->created_at) }}
                                </desc>
                            </div>
                        </div>
                    @endforeach
