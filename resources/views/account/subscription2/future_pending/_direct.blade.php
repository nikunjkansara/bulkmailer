@if (!$subscription->payment_claimed)
    <p class="alert alert-warning">
        {!! trans('messages.subscription.future_pending.' . \horsefly\Model\Setting::get('system.payment_gateway'), [
            'plan' => $subscription->plan->name,
            'money' => horsefly\Library\Tool::format_price($subscription->plan->price, $subscription->plan->currency->format),
        ]) !!}
        <br/>
    </p>
...
@else
    <p class="alert alert-warning">
        {!! trans('messages.subscription.claimed.' . \horsefly\Model\Setting::get('system.payment_gateway'), [
            'plan' => $subscription->plan->name,
            'money' => horsefly\Library\Tool::format_price($subscription->plan->price, $subscription->plan->currency->format),
        ]) !!}
        <br/>
