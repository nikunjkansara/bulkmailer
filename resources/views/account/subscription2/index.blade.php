    ...
    <div class="row">
        <div class="col-sm-12 col-md-6 col-lg-6">
            ...
            @if ($hasPendingPaymentForFuture)
                @include('account.subscription.future_pending._' . \horsefly\Model\Setting::get('system.payment_gateway'))
            @elseif ($goingToExpire && (\horsefly\Model\Setting::get('system.renew_free_plan') == 'yes' || (\horsefly\Model\Setting::get('system.renew_free_plan') == 'no' && !$subscription->plan->isFree())))
                <p class="alert alert-warning mb-40">
                    {!! trans('messages.subscription.renew.warning', [
                        'remain' => $currentPeriodEnd->diffForHumans(),
                        'next_on' => \horsefly\Library\Tool::formatDate($currentPeriodEnd),
                    ]) !!}
                </p>
            @endif
                    ...
                    @if ($subscription->recurring())
                        <p>
                            {!! trans('messages.subscription.current_subscription.recurring.wording', [
                                'plan' => $plan->name,
                                'money' => horsefly\Library\Tool::format_price($plan->price, $plan->currency->format),
                                'remain' => $currentPeriodEnd->diffForHumans(),
                                'next_on' => \horsefly\Library\Tool::formatDate($currentPeriodEnd)
                            ]) !!}
                        </p>
                    ...
                    @elseif ($gateway->isSupportRecurring())
                        <p>
                            {!! trans('messages.subscription.current_subscription.cancel_at_end_of_period.wording', [
                                ...
                                'money' => horsefly\Library\Tool::format_price($plan->price, $plan->currency->format),
                                'remain' => $currentPeriodEnd->diffForHumans(),
                                'end_at' => \horsefly\Library\Tool::formatDate($currentPeriodEnd)
                            ]) !!}
                        </p>
