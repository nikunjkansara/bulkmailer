...
@section('content')
    ...
    <div class="row">
        <div class="col-sm-12 col-md-6 col-lg-6">
            ...
            @if ($gateway->hasError($subscription))
            ...
            <div class="sub-section">
                @if ($subscription->isActive())
                    @if ($subscription->isRecurring())
                        <p>
                            {!! trans('messages.subscription.current_subscription.recurring.wording', [
                                ...
                                'next_on' => \horsefly\Library\Tool::formatDate($subscription->current_period_ends_at)
                            ]) !!}
                        </p>
                    @else
                        <p>
                            {!! trans('messages.subscription.current_subscription.cancel_at_end_of_period.wording', [
                                ...
                                'money' => horsefly\Library\Tool::format_price($plan->price, $plan->currency->format),
                                'remain' => $subscription->current_period_ends_at->diffForHumans(),
                                'end_at' => \horsefly\Library\Tool::formatDate($subscription->current_period_ends_at)
                            ]) !!}
                        </p>
                    @endif
                    ...
                    @if (\Auth::user()->customer->can('renew', $subscription))
                        <a
                            href="{{ $gateway->getRenewUrl($subscription, action('AccountSubscriptionController@index')) }}"
