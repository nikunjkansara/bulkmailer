...
use horsefly\Cashier\Services\BraintreePaymentGateway;
...
class BraintreeController extends Controller
{
        ...
        if ($request->isMethod('post')) {
            ...
            if ($result['amount'] > 0) {
                try {
                    ...
                    $subscription->last_error_type = BraintreePaymentGateway::ERROR_CHARGE_FAILED;
                    $subscription->save();
    ...
    public function renew(Request $request, $subscription_id)
    {
        ...
        if ($request->isMethod('post')) {
            ...
            // Redirect to my subscription page
            return redirect()->action('\horsefly\Cashier\Controllers\BraintreeController@pending', [
