...
class BraintreePaymentGateway implements PaymentGatewayInterface
{
    ...
    public function getCheckoutUrl($subscription, $returnUrl='/') {
        return action("\horsefly\Cashier\Controllers\BraintreeController@checkout", [
    ...
    {
        return action("\horsefly\Cashier\Controllers\\BraintreeController@changePlan", [
            'subscription_id' => $subscription->uid,
            'return_url' => $returnUrl,
            'plan_id' => $plan_id,
        ]);
        ...
        switch ($subscription->last_error_type) {
            case StripePaymentGateway::ERROR_RECURRING_CHARGE_FAILED:
                return trans('cashier::messages.braintree.payment_error.recurring_charge_error', [
                    'url' => action('\horsefly\Cashier\Controllers\BraintreeController@fixPayment', [
                        'subscription_id' => $subscription->uid,
                    ]),
                ]);
