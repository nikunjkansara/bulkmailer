...
namespace horsefly\Cashier\Library\CoinPayment;

use horsefly\Cashier\Library\CoinPayment\CoinpaymentsCurlRequest;
use Exception;
...
class CoinpaymentsAPI
{
     ...
     * @throws Exception
     */
    public function CreateSimpleTransaction($options=[])
    {
        return $this->request_handler->execute('create_transaction', $options);
    }
    ...
    public function GetTxIds($options)
    {
        return $this->request_handler->execute('get_tx_ids', $options);
    }
     ...
     * @throws Exception
     */
    public function GetTxInfoSingle($transaction_id, $full=0)
    {
        $fields = [
            'txid' => $transaction_id,
            'full' => $full,
