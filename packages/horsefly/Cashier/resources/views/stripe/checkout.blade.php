            ...
            <div class="col-md-4 mt-40 pd-60">                
                ...
                @if ($service->getCardInformation($subscription->user) !== NULL)
                    ...
                    <div class="sub-section">
                        ...
                        <a href="{{ action('\horsefly\Cashier\Controllers\StripeController@charge', [
                ...
                <form method="POST" action="{{ action('\horsefly\Cashier\Controllers\StripeController@cancelNow', ['subscription_id' => $subscription->uid]) }}">
                    {{ csrf_field() }}
                ...
                <form id="stripe_button" style="display: none" action="{{ action('\horsefly\Cashier\Controllers\StripeController@updateCard', [
                    ...
                    <input type="hidden" name="redirect" value="{{ action('\horsefly\Cashier\Controllers\StripeController@charge', [
                        'subscription_id' => $subscription->uid,
                    ]) }}" />
                    ...
                    data-name="{{ \horsefly\Model\Setting::get('site_name') }}"
                    data-email="{{ $subscription->user->getBillableEmail() }}"
                    data-description="{{ \horsefly\Model\Setting::get('site_description') }}"
