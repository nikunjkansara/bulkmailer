            ...
            <div class="col-md-4 mt-40 pd-60">
                ...
                <script>
                    var form = jQuery('<form>', {
                        'action': '{{ action('\horsefly\Cashier\Controllers\PaypalSubscriptionController@paymentRedirect') }}',
                        'target': '_top',
                        'method': 'GET'
                    }).append(jQuery('<input>', {
                        'name': '_token',
                    ...
                    paypal.Buttons({
                        ...
                        onApprove: function(data, actions) {
                            ...
                            form.append(jQuery('<input>', {
                                'name': 'redirect',
                                'value': '{{ action('\horsefly\Cashier\Controllers\PaypalSubscriptionController@checkout', $subscription->uid) }}',
                                'type': 'hidden'
                            }));
                            form.submit();
                        }
                ...
                <form class="mt-5" method="POST" action="{{ action('\horsefly\Cashier\Controllers\PaypalSubscriptionController@cancelNow', ['subscription_id' => $subscription->uid]) }}">
                    {{ csrf_field() }}
