...
Route::get('cashier/getURL', function() {
    $path ="test";
    $cashier = new Cashier();
    echo $cashier->public_url($path);
});
...
// UX routes
Route::get('/assets/ux/{name?}', [ function ($name) {
    $path = base_path("resources/views/ux/assets/$name");
    $mime_type = \horsefly\Library\Tool::getFileType($path);
    if (\File::exists($path)) {
...
Route::get('/assets/campaigns/{uid}/{name?}', [ function ($uid, $name) {
    ...
    $mime_type = \horsefly\Library\File::getFileType($path);
    if (\File::exists($path)) {
        return response()->file($path, array('Content-Type' => $mime_type));
    }
...
Route::get('/assets/emails/{uid}/{name?}', [ function ($uid, $name) {
    $email = \horsefly\Model\Email::findByUid($uid);
...
Route::get('contect-us', 'HomeController@contect')->name('contect-us');
...
Route::group(['middleware' => ['not_installed', 'auth', 'frontend', 'subscription']], function () {
    ...
    Route::post('senders/{id}/verify', 'SenderController@verify');
    ...
    Route::get('automation2/{uid}/template/{email_uid}/image', 'EmailController@image');
